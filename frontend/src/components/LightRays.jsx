...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  fragColor.rgb *= 1.9;
  fragColor.a = clamp(max(fragColor.r, max(fragColor.g, fragColor.b)) * 1.05, 0.0, 1.0);
}
...
void main() {
  ...
  gl_FragColor = color;
      ...
      const updatePlacement = () => {
        ...
        const { clientWidth: cssWidth, clientHeight: cssHeight } =
      ...
      cleanupFunctionRef.current = () => {
        ...
        try {
          ...
          // Ignore WebGL teardown errors during fast route/theme changes.
        }
  ...
  return (
    <div
      ref={containerRef}
      className={`relative z-[3] h-full w-full overflow-hidden ${className}`.trim()}
    />
  );
}
